...
// ------------------------------ TRAITEMENT -----------------------------
// 1- restriction d'accès aux administrateur

if (!estAdmin()) { // si membre non admin ou non connecté, on le revoie vers connexion.php
    header('location:../connexion.php'); // attention au ../ 
    exit();
}

// 4- Eregistrement du produit en BDD
//debug($_POST);
...
if (isset($_POST['photo_actuelle'])) { // si existe "photo_actuelle" dans $_POST c'est que nous sommes en train de modifier le produit avec sa photo. On remet l'URL de la photo en BDD :
    ...
    //debug($_FILES); // $_FILES est une superglobal qui représente l'input type "file" d'un formulaire. L'indice "photo" correspond à l'attribut "name" de l'input. Les autres indices du sous-tableau sont prédéfinis :

    // "name" pour le nom de fichier , "type" pour le type du fichier (image), "tmp_name" pour l'adresse du fichier temporaire en cours d'upload, "error" pour le code d'erreur de téléchargement, "size" pour la taille du fichier uploadé.

    if (!empty($_FILES['photo']['name'])) { // si un fichier est en cours d'upload
        $fichier_photo = $_FILES['photo']['name']; // récuperer le nom de la photo

        $photo_bdd = 'photo/' . $fichier_photo; // chemin relatif de la photo qui est enregistré en BDD et qui nous servira pour l'attribut "src" des balises image (les photos sont copiées dans le dossier "photo" ligne suivante)
    ...
    if ($requete) { // si executeRequete a retourné un objet PDOStatement, donc implicitement évalué à true, alors c'est que la requete a marché
        $contenu .= '<div class="alert alert-succes"> Le produit a été enregistré.</div>';
...
<form action="" method="post" enctype="multipart/form-data"> <!-- l'attribut enctype spécifie que le formulaire envoie des données binaires (fichier) et du texte (champ de formulaire) : permet d'uploader une photo -->
    ...
    <div><label for="public">Publique</label></div>
        <div>
    ...
    <!-- 9- Modification de la photos -->
<?php
    if (isset($produit['photo'])) { // si nous somme en train de modifier le produit, nous affichons le photo actuellement en BDD :
