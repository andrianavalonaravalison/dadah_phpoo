...
if (isset($_GET['action']) && $_GET['action'] == 'deconnexion') { // si le membre a cliqué sur "déconnexion"
    // debug($_SESSION);
    unset($_SESSION['membre']); // on ne fait pas un session_destroy() qui supprimerait toutes la session car on veut pouvoir conserver le panier de l'internaute.
...
if (estConnecte()) {
    header('location:profil.php'); // nous affichons le formulaire de connexion qu'aux membre non connectés. Les autre sont rdirigés vers le profil.
...
if (!empty($_POST)) { // si le formulaire a été envoyé
    ...
    if (empty($_POST['pseudo']) || empty($_POST['mdp'])) { // si le champ pseudo ou le champ mdp sont vide ou non définis
        $contenu .= '<div class="alert alert-danger"> Les identifiants et mdp sont obligatoires.</div>';
    }

    // s'il n'y a pas de message d'erreur à l'internaute, on cherche le pseudo en DBB :
    ...
    if (empty($contenu)) { // si c'est vide c'est qu'il n'y a pas d'erreur
        ...
        if ($resultat->rowCount() == 1) { // s'il y a 1 ligne de résultat, alors le pseudo existe : on peut vérifier le mdp
           $membre = $resultat->fetch(PDO::FETCH_ASSOC); // on "fetche" l'objet $resultat pour en extraire les données du membre (pas de boucle car le pseudo est unique)
            ...
            // on vérifie le mdp
            if (password_verify($_POST['mdp'], $membre['mdp'])){ // si le mdp du formulaire correspond au hash du mdo en BDD alors cette fonction retourne true
                // On connecte le membre
                $_SESSION['membre'] = $membre; // nous remplissons la session (session ouverte avec session_start() dans init.php) avec les infos du membre qui proviennent de la BDD

                // puis on redirige vers la page de profil : ity no manipy azy any @ page profil
...
<?php
echo $message; // poue le message de déconnexion
