...
$affiche_formulaire = true; // pour afficher le formulaire tant que le membra n'est pas inscrit
...
if(!empty($_POST)) { // si $_POST n'est pas vide c'est que le formulaire a été envoyé
    // Validation du formulaire : 
        if (!isset($_POST['pseudo']) || strlen($_POST['pseudo']) < 4 || strlen($_POST['pseudo']) > 20  ) { // si le pseudo n'existe pas dans $_POSTc'est que le formulaire est altéré, ou si sa valeur est < 4 ou si sa valeur est > 20 (pour la BDD), on met un message d'erreur à l'internaute
        ...
        { 
            $contenu .= '<div class="alert alert-danger"> Le prenom doit contenir entre 1 et 20 caractères.</div>';
        }

        if (!isset($_POST['email']) ||  !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) || strlen($_POST['email']) > 50 ) 
        { // la fonction prédéfinie filter_var() avec l'argument  FILTER_VALIDATE_EMAIL vérifie que le string fourni est un email
        ...
        if (empty($contenu)) { // si la variable est vide, c'est qu'il n'y a pas de message d'erreur
            ...
            if ($resultat->rowCount() > 0) { // si la requete retourne 1 ou plusieurs lignes c'est que le pseudo est déjà en BDD
                $contenu .= '<div class="alert alert-danger"> Pseudo indisponible. Veuillez en choisir un autre.</div>';
            } else {
                // sinon on fait l'insertion en BDD
                $mdp = password_hash($_POST['mdp'], PASSWORD_DEFAULT); // nous hachons le mot de passe avec l'agorithme bccrypt par défaut qui nous retourne une clé de hachage. Nous allons l'insérer en BDD.
                // debug($mdp);
                $succes = executeRequete(
                "INSERT INTO membre (pseudo, mdp, nom, prenom, email, civilite, ville, code_postal, adresse, statut) VALUES (:pseudo, :mdp, :nom, :prenom, :email, :civilite, :ville, :code_postal, :adresse, :statut)", array(':pseudo'      => $_POST['pseudo'],
                                                         ...
                                                         ':statut'      => 0 // pour un membre non administrateur 
                ));
...
if ($affiche_formulaire) : // syntaxe en if () : .... endif; ou le ":" correspond à l'accolade ouvrante et le endif à la fermante. Si le membre n'est pas inscrit, on lui affiche le formulaire.
...
?>
    ...
    <form method="post" action="">
        ...
        <div><label for="prenom">Prénoom</label></div>
