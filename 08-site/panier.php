...
// ---------------------------- TRAITEMENT -------------------------
//debug($_POST); // pour vérifier la quantité des produit dans le panier
...
if (!empty($_POST)) { // si le formulaire d'ajout au panier a été envoyé
    ...
    // On remplie la session avec les infos du produit ajouté au panier (on les met dans la session pour que le panier soit accessible dans toutes les pages du site)

    $_SESSION['panier']['id_produit'][] = $_POST['id_produit']; // id_produit vient du formulaire donc de $_POST. Les [] permet d'ajouter un élément à la FIN du tableau avec de indice numérique.
    ...
    header('location:fiche_produit.php?id_produit=' . $_POST['id_produit']); // je passe l'identifiant produit dans l'URL pour réafficher la fiche de produit
    exit();
