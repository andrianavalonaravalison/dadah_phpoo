...
if (!empty($_POST)) { // si le formulaire a été envoyé
	...
	if (!isset($_POST['cp']) || !preg_match('#^[0-9]{5}$#', $_POST['cp'])) {
		...
		// surface
    if(!isset($_POST['surface']) || strlen($_POST['surface']) > 10  ) {
            $contenu .= '<div class="table-danger" scope="row"> La surface doit être supérieur à 10m² </div>';
    }

    	// Description (je vais limiter quand même à 255 caractères)
	...
	if (empty($contenu)) {
		...
		$resultat = $pdo->prepare("INSERT INTO logements(titre, adresse, ville, cp, surface, prix, photo, type_bien, description) VALUES (:titre, :adresse, :ville, :cp, :surface, :prix, :photo, :type_bien, :description)");
...
<!-- Création du formulaire HTML afin d'ajouter un contact dans la bdd. Le champ type_contact doit être géré via un "select option". On doit pouvoir uploader une photo par le formulaire. -->
